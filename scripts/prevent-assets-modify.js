...
try {
  ...
  if (offending.length) {
    ...
    console.error(
      "Si vous avez besoin d’exceptions, contactez l’équipe et appliquez un commit explicite après vérification."
    );
    process.exit(1);
  }
  process.exit(0);
  ...
  console.error(
    "Erreur lors de la vérification des fichiers git:",
    e && e.message
  );
