...
export function generateCanonicalThemeFromFigma({
  ...
  const collectVar = (v) => {
    if (!v || !v.id) return;
    idIndex[v.id] = v;
    if (v.name) nameIndex[String(v.name)] = v;
  };
  ...
  const resolveVarRgb = (v, visited = new Set()) => {
    ...
    if (entry.aliasName && nameIndex[entry.aliasName]) {
      return resolveVarRgb(nameIndex[entry.aliasName], visited);
    }
  ...
  for (const v of primitives.variables || []) {
    ...
    if (v.type === "COLOR") {
      ...
      if (css) colors.push({ name, css });
      else continue; // skip non-resolvable color primitives
