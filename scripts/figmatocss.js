...
const { ensureDir, writeFile, readJsonFiles, clampBetweenModes } = require("./utils");
const colors = require("./extract/colors");
const spacing = require("./extract/spacing");
const fonts = require("./extract/fonts");

const SRC_DIR = process.argv[2] || path.join(process.cwd(), "source");
const OUT_DIR = process.argv[3] || path.join(process.cwd(), "dist");
...
(async function main() {
  ...
  if (!files.length) {
    console.warn("Aucun fichier JSON trouvé dans", SRC_DIR, "— génération des valeurs globales par défaut seulement.");
  }
...
`;
  ...
  function normalizeTokens(rawColors, rawSpacing, rawFonts, primitivesFlat) {
    const normalized = { colors: {}, spacing: {}, fonts: { fontSize: {}, lineHeight: {} } };
    ...
    Object.keys(rawColors || {}).forEach((tokenName) => {
      ...
      const modes = {};
      if (tokenRaw.light && tokenRaw.light.value) modes.light = tokenRaw.light.value;
    ...
    Object.keys(rawSpacing || {}).forEach((k) => {
      ...
      if (structuredPrimitives && structuredPrimitives.spacing && structuredPrimitives.spacing[clean]) return;
      if (structuredPrimitives && structuredPrimitives.rounded && structuredPrimitives.rounded[clean]) return;
      ...
      if (mode) {
        ...
        let found = keys.find(
          (pk) =>
            ...
            (Number(primitivesLookup[pk]) === Number(val) || String(primitivesLookup[pk]) === String(val)),
        );
        if (!found) {
          // Fallback: match any primitive with the same value
          found = keys.find(
            (pk) => Number(primitivesLookup[pk]) === Number(val) || String(primitivesLookup[pk]) === String(val),
          );
        }
        if (found) {
  ...
  if (colorResult.warnings && colorResult.warnings.length)
    warnings.push(...colorResult.warnings.map((w) => ({ source: "colors", ...w })));
  if (fontResult.warnings && fontResult.warnings.length)
    warnings.push(...fontResult.warnings.map((w) => ({ source: "fonts", ...w })));
  if (normalizedTokens._warnings && normalizedTokens._warnings.length)
    warnings.push(...normalizedTokens._warnings.map((w) => ({ source: "normalize", ...w })));
  ...
  function primitiveExists(varName) {
    ...
    if (
      ...
      return true;
    if (finalStructuredPrimitives.spacing && finalStructuredPrimitives.spacing[k]) return true;
    if (finalStructuredPrimitives.fontSize && finalStructuredPrimitives.fontSize[k]) return true;
    if (finalStructuredPrimitives.lineHeight && finalStructuredPrimitives.lineHeight[k]) return true;
    if (finalStructuredPrimitives.rounded && finalStructuredPrimitives.rounded[k]) return true;
    ...
    const tokenCheck = normalizedTokens.colors && normalizedTokens.colors[k.replace(/^color-/, "")] ? true : false;
  ...
  Object.keys((normalizedTokens.fonts && normalizedTokens.fonts.fontSize) || {}).forEach((k) => {
  ...
  Object.keys((normalizedTokens.fonts && normalizedTokens.fonts.lineHeight) || {}).forEach((k) => {
  ...
  if (warnings.length) {
    ...
    console.log("\nWarnings:");
    warnings.slice(0, 20).forEach((w) => console.log(`- [${w.source}] ${w.type}: ${w.message}`));
  ...
  if (!entries.length) {
    ...
    Object.keys(globalColorDefaults).forEach((name) => {
      if (!finalStructuredPrimitives.color[name]) {
        finalStructuredPrimitives.color[name] = { $type: "color", value: globalColorDefaults[name] };
    ...
    Object.keys(spacingDefaults).forEach((k) => {
      if (!finalStructuredPrimitives.spacing[k]) {
        finalStructuredPrimitives.spacing[k] = { $type: "number", value: spacingDefaults[k] };
        // store px value for numeric computations
        allPrimitivesFlat[`--${k}`] = Math.round(parseFloat(spacingDefaults[k]) * 16);
      }
    });
    ...
    Object.keys(fontSizeDefaults).forEach((k) => {
      if (!finalStructuredPrimitives.fontSize[k]) {
        finalStructuredPrimitives.fontSize[k] = { $type: "number", value: fontSizeDefaults[k] };
        // store px value for numeric computations (e.g., clamp calculation)
        allPrimitivesFlat[`--${k}`] = Math.round(parseFloat(fontSizeDefaults[k]) * 16);
      }
    });
    ...
    Object.keys(radiusDefaults).forEach((r) => {
      if (!finalStructuredPrimitives.rounded[r]) {
        finalStructuredPrimitives.rounded[r] = { $type: "number", value: radiusDefaults[r] };
      }
    });

    // Add a few simple primitives expected by the theme generator
    if (!finalStructuredPrimitives["font-base"])
      finalStructuredPrimitives["font-base"] = { $type: "string", value: "system-ui, sans-serif" };
    if (!finalStructuredPrimitives["font-weight-regular"])
      finalStructuredPrimitives["font-weight-regular"] = { $type: "number", value: 400 };
  }
  ...
  function findProjectColorValue(key) {
    if (!finalStructuredPrimitives || !finalStructuredPrimitives.color) return null;
    if (finalStructuredPrimitives.color[key]) return finalStructuredPrimitives.color[key].value;
    // try neutral-* variant for gray-*
    const m = key.match(/^gray-(\d+)$/);
    if (m) {
      const alt = `neutral-${m[1]}`;
      if (finalStructuredPrimitives.color[alt]) return finalStructuredPrimitives.color[alt].value;
      const alt2 = `slate-${m[1]}`;
      if (finalStructuredPrimitives.color[alt2]) return finalStructuredPrimitives.color[alt2].value;
    }
    return null;
  }
  ...
  const projectColorNames = Object.keys(finalStructuredPrimitives.color || {}).filter((n) => {
  ...
  const colorsLines = projectColorNames
    .sort()
    .map((name) => `--color-${name}: ${finalStructuredPrimitives.color[name].value};`);
  ...
  const lineHeightLines = Object.keys(finalStructuredPrimitives.lineHeight || {})
    .sort((a, b) => {
  ...
  const colorsSection = ["/* Colors */", ...colorsLines, ...colorResult.tokensCss];
  const spacingSection = ["/* Spacing */", ...radiusLines, ...spacingLines];
  const typographySection = ["/* Typography */", ...textLines, ...lineHeightLines];
  const typographyTokensSection = ["/* Typography Tokens */", ...(fontResult.tokensCss || [])];
  ...
  function computeFluidClamp(mobileRef, desktopRef) {
    ...
    function numericPxFromRef(ref) {
      ...
      if (typeof ref === "string") {
        ...
        if (varMatch) {
          ...
          if (finalStructuredPrimitives.fontSize && finalStructuredPrimitives.fontSize[key]) {
            return parseFloat(finalStructuredPrimitives.fontSize[key].value.replace(/rem$/, "")) * 16;
          }
          if (finalStructuredPrimitives.lineHeight && finalStructuredPrimitives.lineHeight[key]) {
            return parseFloat(finalStructuredPrimitives.lineHeight[key].value.replace(/rem$/, "")) * 16;
          }
          if (finalStructuredPrimitives.spacing && finalStructuredPrimitives.spacing[key]) {
            return parseFloat(finalStructuredPrimitives.spacing[key].value.replace(/rem$/, "")) * 16;
          }
          if (finalStructuredPrimitives.rounded && finalStructuredPrimitives.rounded[key]) {
            return parseFloat(finalStructuredPrimitives.rounded[key].value.replace(/rem$/, "")) * 16;
          }
        }
      }
      return null;
    ...
    if (mobilePx === null || desktopPx === null || Number.isNaN(mobilePx) || Number.isNaN(desktopPx)) {
    ...
    const left = mobileExpr && mobileExpr.startsWith("var(") ? mobileExpr : pxToRem(mobilePx);
    const right = desktopExpr && desktopExpr.startsWith("var(") ? desktopExpr : pxToRem(desktopPx);
  ...
  const globalColorsSection = ["/* Couleurs (globales) */", ...globalColorsLines];
  const projectColorsSection = colorsLines.length ? ["/* Couleurs Primitives du projet */", ...colorsLines] : [];
  ...
  const colorTokenGroups = [
    { comment: "/* Couleur primaire */", keys: ["primary", "on-primary", "primary-lighten", "primary-darken"] },
    { comment: "/* Couleur d'accent */", keys: ["accent", "accent-invert"] },
    { comment: "/* Surface du document */", keys: ["surface", "on-surface"] },
    { comment: "/* Niveaux de profondeur */", keys: ["layer-1", "layer-2", "layer-3"] },
    { comment: "/* Interactions */", keys: ["link", "link-hover", "link-active"] },
    { comment: "/* Couleur de sélection */", keys: ["selection"] },
    { comment: "/* États d'alerte */", keys: ["warning", "error", "success", "info"] },
  ...
  colorTokenGroups.forEach((group, idx) => {
    ...
    group.keys.forEach((name) => {
      ...
      const nt = (normalizedTokens && normalizedTokens.colors && normalizedTokens.colors[name]) || null;
      if (nt) {
        if (nt.modes && nt.modes.light && nt.modes.dark) {
          globalColorTokensLines.push(`--${name}: light-dark(${nt.modes.light}, ${nt.modes.dark});`);
          return;
        }
        if (nt.value) {
      ...
      if (finalStructuredPrimitives.color && finalStructuredPrimitives.color[name]) {
        globalColorTokensLines.push(`--${name}: ${finalStructuredPrimitives.color[name].value};`);
        return;
      }

      globalColorTokensLines.push(`--${name}: ${globalColorTokenDefaults[name]};`);
    });
    if (idx !== colorTokenGroups.length - 1) globalColorTokensLines.push(""); // blank line between groups
  });

  const globalColorTokensSection = ["/* Couleurs Tokens globales */", ...globalColorTokensLines];
  ...
  Object.keys(colorTokens)
    ...
    .forEach((name) => {
      ...
      if (entry && entry.modes && entry.modes.light && entry.modes.dark) {
        projectColorTokensLines.push(`--${name}: light-dark(${entry.modes.light}, ${entry.modes.dark});`);
  ...
  function getProjectPrimitiveValue(name) {
    if (allPrimitivesFlat && allPrimitivesFlat[`--${name}`]) return allPrimitivesFlat[`--${name}`];
    if (
  ...
  ["transition-duration"].forEach((k) => {
    ...
    otherPrimitivesLines.push(`--${k}: ${projectVal || otherPrimitivesDefaults[k]};`);
  });
  otherPrimitivesLines.push("");

  otherPrimitivesLines.push("/* Niveaux de z-index */");
  ["z-under-page-level", "z-above-page-level", "z-header-level", "z-above-header-level", "z-above-all-level"].forEach(
    (k) => {
      const projectVal = getProjectPrimitiveValue(k);
      otherPrimitivesLines.push(`--${k}: ${projectVal || otherPrimitivesDefaults[k]};`);
    },
  ...
  const otherPrimitivesSection = ["/* Autres Primitives CSS globales */", ...otherPrimitivesLines];
  ...
  const spacingPrimitivesSection = spacingLines.length ? [spacingTitle, ...spacingLines] : [];
  ...
  Object.keys(spacingTokens)
    ...
    .forEach((k) => {
      ...
      if (entry.modes && entry.modes.mobile && entry.modes.desktop) {
        ...
        projectSpacingTokensLines.push(`--${clean}: ${computeFluidClamp(mobile, desktop)};`);
      } else if (entry.value) {
  ...
  const typographyPrimitivesSection =
    textLines.length || lineHeightLines.length ? [typographyTitle, ...textLines, ...lineHeightLines] : [];
  ...
  const lineTokensNorm = (normalizedTokens.fonts && normalizedTokens.fonts.lineHeight) || {};
  ...
  (fontResult.tokensCss || []).forEach((line) => {
    ...
    if (l.includes("NaN") || l.includes("calc(")) {
      ...
      if (norm && norm.modes && norm.modes.mobile && norm.modes.desktop) {
        typographyTokensLines.push(`--${tokenName}: ${computeFluidClamp(norm.modes.mobile, norm.modes.desktop)};`);
        return;
      }
  ...
  Object.keys(fontTokensNorm)
    ...
    .forEach((k) => {
      ...
      if (entry.modes && entry.modes.mobile && entry.modes.desktop) {
        ...
        if (!existing)
          typographyTokensLines.push(`--${name}: ${computeFluidClamp(entry.modes.mobile, entry.modes.desktop)};`);
      } else if (entry.modes && entry.modes.light && entry.modes.dark) {
        const existing = findExtractorTokenLine(name);
        if (!existing) typographyTokensLines.push(`--${name}: light-dark(${entry.modes.light}, ${entry.modes.dark});`);
      } else if (entry.value) {
        const existing = findExtractorTokenLine(name);
  ...
  Object.keys(lineTokensNorm)
    ...
    .forEach((k) => {
      ...
      if (entry.modes && entry.modes.mobile && entry.modes.desktop) {
        typographyTokensLines.push(`--${name}: ${clampBetweenModes(entry.modes.mobile, entry.modes.desktop)};`);
      } else if (entry.modes && entry.modes.light && entry.modes.dark) {
        typographyTokensLines.push(`--${name}: light-dark(${entry.modes.light}, ${entry.modes.dark});`);
      } else if (entry.value) {
  ...
  typographyTokensLines.forEach((l) => {
    ...
    const existingPreferred = /var\(/.test(existing) && /rem\s*\+\s*[0-9.]+vw/.test(existing);
  ...
  Object.keys(fontTokensNorm).forEach((k) => {
    ...
    if (entry && entry.modes && entry.modes.mobile && entry.modes.desktop) {
      tokenLineMap[name] = `--${name}: ${computeFluidClamp(entry.modes.mobile, entry.modes.desktop)};`;
    }
  });
  ...
  const dedupedTypographyTokensLines = [...textTokenObjs.map((o) => o.line), ...lhTokenObjs.map((o) => o.line)];
  ...
  const rootBody = [
    ...
    projectSpacingTokensSection.length ? indent(projectSpacingTokensSection) : null,
    "",
    indent(typographyPrimitivesSection),
    projectTypographyTokensSection.length ? "" : null,
    projectTypographyTokensSection.length ? indent(projectTypographyTokensSection) : null,
  ]
    .filter(Boolean)
    .join("\n\n");

  const cssPartsFinal = [header, breakpointsBlock, ":root {", rootBody, "}"].join("\n\n");
  ...
  writeFile(path.join(OUT_DIR, "primitives.json"), JSON.stringify(finalStructuredPrimitives, null, 2));
  writeFile(path.join(OUT_DIR, "tokens.json"), JSON.stringify(normalizedTokens, null, 2));

  // Summary report
  const filesCount = entries.length;

  // Final counts (after normalization & potential primitive creation)
  const colorPrimitivesCount = Object.keys(finalStructuredPrimitives.color || {}).length;
  const colorTokensCount = Object.keys(normalizedTokens.colors || {}).length;
  const spacingPrimitivesCount = Object.keys(finalStructuredPrimitives.spacing || {}).length;
  const spacingTokensCount = Object.keys(normalizedTokens.spacing || {}).length;
  const roundedPrimitivesCount = Object.keys(finalStructuredPrimitives.rounded || {}).length;
  ...
  const typographyTokensCount =
    Object.keys((normalizedTokens.fonts && normalizedTokens.fonts.fontSize) || {}).length +
    Object.keys((normalizedTokens.fonts && normalizedTokens.fonts.lineHeight) || {}).length;

  // Extractor-level counts (what each extractor discovered before normalization)
  const colorExtractorPrimitivesCount = Object.keys(colorResult.primitivesJson || {}).length;
  const colorExtractorTokensCount = Object.keys(colorResult.tokensJson || {}).length;
  const spacingExtractorPrimitivesCount = Object.keys(spacingResult.primitives || {}).length;
  const spacingExtractorTokensCount = Object.keys(spacingResult.json || {}).length;
  const roundedExtractorPrimitivesCount = spacingExtractorPrimitivesCount
    ? Object.keys(spacingResult.primitives || {}).filter((k) => k.startsWith("--radius-")).length
    : 0;
  const fontExtractorPrimitivesCount = Object.keys(fontResult.primitives || {}).length;
  const fontExtractorTokensCount = Object.keys(
    fontResult.json && (fontResult.json.fontSize || {}) && (fontResult.json.lineHeight || {})
  ...
  console.log(`- Fichiers corrigés finalisés : primitives.json, tokens.json, theme.css`);
