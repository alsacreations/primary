...
async function main() {
  ...
  try {
    ...
    console.log(
      "[debug-font-poppins] state.tokensContent length:",
      state.tokensContent.length
    );
    console.log("[debug-font-poppins] state.config ->", state.config);

    const out = generateTokensCSS();
    await fs.writeFile(
      path.join(process.cwd(), "tmp", "debug-font-poppins.css"),
      out,
      "utf8"
    );
    console.log("WROTE tmp/debug-font-poppins.css");
  } catch (e) {
